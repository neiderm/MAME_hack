 ...
 *
 * $Id: gmplayer.c,v 1.32.2.4 1998/02/27 20:04:43 pekangas Exp $
 *
...
RCSID(const char *gmplayer_rcsid = "$Id: gmplayer.c,v 1.32.2.4 1998/02/27 20:04:43 pekangas Exp $";)
...
*                   int restartPos, int loopSong, int numChannels
...
*               int loopSong            should song playback be looped?
...
int CALLING gmpPlaySong(gmpModule *module, int startPos, int endPos,
    int restartPos, int loopSong, int numChannels, unsigned *sdChannels,
    gmpPlayHandle *playHandle)
{
    ...
    handle->loopSong = loopSong;
    handle->songLoopCount = 0;
    handle->userMasterVolume = 64;
...
int CALLING gmpFreeModule(gmpModule *module)
{
    ...
    if ( module->instruments != NULL )
    {
        for ( i = 0; i < module->numInsts; i++ )
        {
            ...
            if ( inst != NULL )
            {
                ...
                DEBUGprintf("inst[%i]->pitchEnvelope\n", i);
                /* If the instrument has pitch envelope, deallocate it: */
                if ( inst->pitchEnvelope != NULL )
                {
                    if ( (error = memFree(inst->pitchEnvelope)) != OK )
                        PASSERROR(ID_gmpFreeModule)
                }
    ...
    /* Deallocate samples if allocated: */
    if ( module->samples != NULL )
    {
        for ( i = 0; i < module->numSamples; i++ )
        {
            smp = module->samples[i];
            if ( smp != NULL )
            {
                ...
                /* Deallocate the sample structure: */
                if ( (error = memFree(smp)) != OK )
                    PASSERROR(ID_gmpFreeModule)
            }
        }
        
        DEBUGputs("module->samples");
        /* Deallocate instrument structure pointers: */
        if ( (error = memFree(module->samples)) != OK )
            PASSERROR(ID_gmpFreeModule)
    }
...
int CALLING gmpPlay(void)
{
    ...
    /* Check that we can actually play: */
    if ( gmpNoPlay )
        return OK;
    ...
    for ( handleNum = 0; handleNum < MAXSONGS; handleNum++ )
    {
        if ( playHandles[handleNum] != NULL )
        {
            ...
            if ( (handle->songLoopCount > 0) && (handle->loopSong == 0) )
                continue;
    ...
    gmpInformation **information)
{
    ...
    playHandle->information->songLoopCount = playHandle->songLoopCount;
...
int gmpCalcEnvelope(gmpEnvelope *env, gmpEnvStatus *stat)
{
    int point, i;
    int x, y, tx, ty;
    ...
    if ( stat->x == env->points[point].x )
    {
        y = env->points[point].y * 256;
    }
    else
    {
        y = env->points[point - 1].y;
        ty = env->points[point].y - y;
        x = env->points[point - 1].x;
        tx = env->points[point].x - x;
        y *= 256;
        y += ty * 256 / tx * ( stat->x - x );
    }

    if ( ( stat->x == env->points[env->sustain].x) &&
         (gmpChan->keyOff == 0) && ( env->sustain != -1 ) )
        stat->sustained = 1;

    if ( !stat->sustained  )
    {
        if ( stat->x < env->points[env->numPoints - 1].x )
            stat->x++;

        if ( (stat->x >= env->points[env->loopEnd].x )&&
             (env->loopEnd != -1 ) )
            stat->x = env->points[env->loopStart].x;
        ...
        {
            if ( ( stat->x == env->points[env->susLoopEnd].x) &&
               (gmpChan->keyOff == 0) && ( env->sustain != -1 ) )
                stat->x = env->points[env->susLoopStart].x;
        }
    }
    return y;
}
        ...
        switch ( gmpPlayMode )
        {
            case gmpFT2:
                ...
                if ( gmpChan->instrument != -1  )
                {
                    ...
                    {
                        vol *= gmpCalcEnvelope(inst->volEnvelope, &gmpChan->volEnv);
                        vol >>= 14;
                        if ( vol < 0 ) vol = 0;
                        if ( vol > 64 ) vol = 64;
                    }
                ...
                vol = (vol * playHandle->masterVolume * gmpChan->volFade *
                       playHandle->userMasterVolume + ((1 << 18) - 1)) >> 18;
                ...
                if ( gmpChan->instrument != -1 )
                {
                    ...
                    {
                        val = gmpCalcEnvelope(inst->panEnvelope, &gmpChan->panEnv);
            ...
            case gmpIT:
                ...
                if ( gmpChan->instrument != -1 )
                {
                    ...
                    vol = (vol * gmpChan->fadeOut) >> 9;
                    if ( gmpChan->keyOff != 0)
                    {
                        gmpChan->fadeOut -= inst->volFadeout;
                        if ( gmpChan->fadeOut < 0 ) gmpChan->fadeOut = 0;
                    }
                ...
                if ( gmpChan->instrument != -1 )
                {
                    ...
                    if ( ((env = inst->panEnvelope) != NULL) &&
                         (inst->panEnvelope->numPoints > 0) )
                    {
                        pan += gmpCalcEnvelope(inst->panEnvelope, &gmpChan->panEnv);
                ...
                /* Set panning to Sound Device: */
                if ( (error = gmpSD->SetPanning(gmpChan->sdChannel, pan))
                     != OK )
            ...
            default:
                vol = (gmpChan->realVolume * playHandle->masterVolume *
                       gmpChan->volFade * playHandle->userMasterVolume +
                       ((1 << 18) - 1)) >> 18;
                gmpChan->trueVolume = (uchar) vol;
...
int gmpPlayPattern(gmpPlayHandle playHandle)
{
    ...
    switch ( gmpPlayMode )
    {
        case gmpPT:
        default:
            Commands = gmpTick0CommandsPT;
            break;

        case gmpFT2:
            Commands = gmpTick0CommandsFT2;
            break;

        case gmpST3:
            Commands = gmpTick0CommandsST3;
            break;

        case gmpIT:
            Commands = gmpTick0CommandsIT;
            break;
    }
    ...
    for ( chanNum = 0; chanNum < playHandle->numChannels; chanNum++ )
    {
        ...
        if ( gmpChan->status.newInst )
        {
            switch ( gmpPlayMode )
            {
                ...
                case gmpST3 :                    
#ifdef M_TRACKER
                    /* Don't get confused if we are playing non-existing
                       instruments or samples: */
                    if ( (gmpCurModule->instruments[gmpChan->newInstrument] == NULL) ||
                         (gmpCurModule->instruments[gmpChan->newInstrument]->numSamples == 0) ||
                         (gmpCurModule->instruments[gmpChan->newInstrument]->samples[0] == NULL) )
                    {
                        gmpChan->instrument = -1;
                        gmpChan->sample = 0xFF;
                ...
                case gmpFT2:
                    /* FT2 always retriggers the envelope in note
                       delay command: */
                    if ( !((gmpChan->status.command) &&
                           (gmpChan->command == gmpcNoteDelay)) )
                    {
                        gmpChan->volEnv.x = 0;
                        gmpChan->volEnv.sustained = 0;
                        gmpChan->panEnv.x = 0;
                        gmpChan->panEnv.sustained = 0;
                            ...
                            (gmpChan->command != gmpcTonePortamento)) )
                    {
                        ...
                        if ( xmNote < 0xFE )
                        {
                            ...
                            if ( (instStruct->numSamples <= gmpChan->sample) ||
                                 (instStruct->samples[gmpChan->sample] == NULL) )
                            {
                                gmpChan->sample = 0xFF;
                            }
                        }
                ...
                case gmpIT:
                    ...
                    gmpChan->panEnv.x = 0;
                    gmpChan->panEnv.sustained = 0;
                    gmpChan->pitchEnv.x = 0;
                    gmpChan->pitchEnv.sustained = 0;
                    gmpChan->fadeOut = 512;
                    gmpChan->autoVibPos = 0;
                    gmpChan->autoVibDepth = 0;
                    ...
                    /* We'll only actually change the instrument if there is
                       a new note: */
                    if ( (gmpChan->status.newNote) && (gmpChan->note < 0xFD)
                        && ((!gmpChan->status.command) ||
                            (gmpChan->command != gmpcTonePortamento)) )
                    {
                        gmpChan->instrument = gmpChan->newInstrument;
                        ...
                        if ( xmNote < 0xFD )
                        {
                            ...
                            {
                                gmpChan->sample =
                                    instStruct->noteSamples[xmNote*2+1];
                                xmNote = instStruct->noteSamples[xmNote*2];
                                gmpChan->note = (xmNote % 12)|((xmNote/12)<<4);
                            }
                            else
                                gmpChan->sample = 0;
                    ...
                    if ( gmpChan->sample != 0xff )
                    {
                        ...
                        /* Set instrument panning as current channel panning: */
                        if ( !(instStruct->DfP & 0x80 ) )
                            gmpSetPanning(instStruct->DfP);
        ...
        if ( gmpChan->status.newNote )
        {
            if ( ((gmpPlayMode == gmpFT2) || (gmpPlayMode == gmpIT)) &&
                 (gmpChan->instrument != -1) )
            {
                /* Point instStruct to new instrument: */
                instStruct = gmpCurModule->instruments[gmpChan->instrument];
                ...
                if ( xmNote < 0xFD )
                {
                    ...
                    if ( instStruct->noteSamples != NULL )
                    {
                        ...
                        {
                            gmpChan->sample = instStruct->noteSamples[xmNote*2+1];
                            xmNote = instStruct->noteSamples[xmNote*2];
                            gmpChan->note = (xmNote % 12)|((xmNote/12)<<4);
                        }
...
/*
        printf("C:%i, per: %i, vol: %i, pan: %i, note: %i, inst: %i, "\
               "cmd: %i, info: %i, volc: %i\n",
                    chanNum,
                    gmpChan->period,
                    gmpChan->volume,
    ...
    if ( gmpHandle->playPtr != NULL )
    {
            ...
            gmpCurModule->patterns[gmpHandle->pattern]->rows )
        {
            ...
            if ( gmpPlayMode == gmpST3 || gmpPlayMode == gmpIT )
                gmpHandle->loopRow = 0;
            ...
            do
            {
                    ...
                    (gmpCurModule->songData[gmpHandle->position] == 0xffff))
                {
                    ...
                    gmpHandle->songLoopCount++;  /* the song just looped */
                    gmpHandle->masterVolume = gmpCurModule->masterVolume;
                }
...
int gmpHandleCommands(gmpPlayHandle playHandle)
{
    ...
    switch ( gmpPlayMode )
    {
        case gmpPT :
        default:
            Commands = gmpContCommandsPT;
            break;

        case gmpFT2 :
            Commands = gmpContCommandsFT2;
            break;

        case gmpST3 :
            Commands = gmpContCommandsST3;
            break;

        case gmpIT :
            Commands = gmpContCommandsIT;
            break;
    }
...
//                Period = 10*12*16*4 - Note*16*4 - FineTune/2;
        ...
        case gmpIT:
            ...
            *period = ( ( 65536 * 4 * ( gmpPeriodsST3[note & 0xF] ) )
...
//                Frequency = 8363*2^((6*12*16*4 - Period) / (12*16*4));
            ...
            /* Not sure whether this is a good idea, but seems to work: */
        case gmpIT:
...
int gmpSetPanning(int panning)
{
    ...
    switch ( gmpPlayMode )
    {
        ...
        case gmpIT:
            ...
            if ( (error = gmpSD->SetPanning(gmpChan->sdChannel, (panning*2)-64))
                != OK )
                PASSERROR(ID_gmpSetPanning)
            break;
    ...
    if ( (gmpChan->instrument != -1) && (gmpChan->sample != 0xFF) )
    {
        ...
        switch ( gmpChan->note )
        {
                ...
                switch ( gmpPlayMode )
                {
                    case gmpFT2:
                        if ( ((gmpCurModule->instruments[gmpChan->instrument]
                               ->volEnvelope == NULL) ||
                              (gmpCurModule->instruments[gmpChan->instrument]
                               ->volEnvelope->numPoints == 0))
                             &&
            ...
            default:
                ...
                {
                    /* FT2 and IT: If the current sample is a 16-bit one,
                       multiply start offset by 2: */
                    if ( gmpCurModule->instruments[gmpChan->instrument]
                        ->samples[gmpChan->sample]->sampleType == smp16bitMono )
                    {
                        if ( (error = gmpSD->SetPosition(gmpChan->sdChannel,
                            2 * gmpChan->startOffset)) != OK)
                            PASSERROR(ID_gmpNewNote)
                    }
                    else
                    {
...
/****************************************************************************\
*
* Function:     int gmpFadeChannel(gmpPlayHandle playHandle,
*                   unsigned channel, unsigned fade)
*
* Description:  Sets a channel fade value
*
* Input:        gmpPlayHandle playHandle  the playing handle for the module
*               unsigned channel        music channel number
*               unsigned fade           fade: 64 is normal volume, 0 is
*                                       complete fade out
*
* Returns:      MIDAS error code
*
...
int CALLING gmpFadeChannel(gmpPlayHandle playHandle, unsigned channel,
                           unsigned fade)
{
    if ( channel >= playHandle->numChannels )
    {
        ERROR(errInvalidChanNumber, ID_gmpFadeChannel);
        return errInvalidChanNumber;
    }

    playHandle->channels[channel].volFade = fade;
...
/****************************************************************************\
*
* Function:     int gmpSetMasterVolume(gmpPlayHandle playHandle,
*                   unsigned volume)
*
* Description:  Sets playback master volume
*
* Input:        gmpPlayHandle playHandle   the playing handle
*               unsigned volume         master volume (0-64)
*
* Returns:      MIDAS error code
*
\****************************************************************************/

int CALLING gmpSetMasterVolume(gmpPlayHandle playHandle, unsigned volume)
{
    playHandle->userMasterVolume = (uchar) volume;
...
/*
 * $Log: gmplayer.c,v $
 * Revision 1.32.2.4  1998/02/27 20:04:43  pekangas
 * Made note delay more FT2 compatible
 *
 * Revision 1.32.2.3  1998/01/01 17:56:23  pekangas
 * A small key off fix
 *
 * Revision 1.32.2.2  1997/11/18 19:35:09  pekangas
 * Instruments without envelopes can now have the envelope structures
 * allocated, if they have numPoints=0
 * gmplayer no longer crashes with modules that play non-existing samples or
 * instruments
 *
 * Revision 1.32.2.1  1997/09/22 14:49:00  pekangas
 * Module sample pointers are also now deallocated
 *
 * Revision 1.32  1997/08/17 12:34:48  pekangas
 * Added proper user master volume and gmpSetMasterVolume()
 *
 * Revision 1.31  1997/07/31 10:56:41  pekangas
 * Renamed from MIDAS Sound System to MIDAS Digital Audio System
 *
 * Revision 1.30  1997/07/14 08:04:28  jpaana
 * Misc. changes for better IT-compatibility
 *
 * Revision 1.29  1997/07/01 23:32:47  jpaana
 * Added sustain loops to envelopes
 *
 * Revision 1.28  1997/07/01 23:23:18  jpaana
 * Added envelopes and fadeouts to IT
 *
 * Revision 1.27  1997/06/30 12:24:41  pekangas
 * Fixed sample deallocation - nonexisting samples were being freed
 *
 * Revision 1.26  1997/06/30 11:34:13  pekangas
 * Fixed a typo
 *
 * Revision 1.25  1997/06/30 10:59:50  jpaana
 * Major rewrite on some parts for better IT-support
 *
 * Revision 1.24  1997/06/23 00:03:50  jpaana
 * Fixed panning in IT-mode
 *
 * Revision 1.23  1997/06/22 18:09:24  jpaana
 * Made samples independent from instruments. A rather fundamental change, but
 * necessary for .it-support in instrument mode.
 *
 * Revision 1.22  1997/06/11 14:47:11  jpaana
 * Some cosmetic fixes to keep Prolint happy
 *
 * Revision 1.21  1997/06/11 03:15:18  jpaana
 * Some cleanup
 *
 * Revision 1.19  1997/06/10 19:52:33  jpaana
 * Added IT-loader
 *
 * Revision 1.18  1997/05/20 10:16:28  pekangas
 * Added preliminary tracker mode support
 *
 * Revision 1.17  1997/05/19 13:25:40  pekangas
 * Fixed sample offsets with 16-bit samples
 *
 * Revision 1.16  1997/05/02 13:18:05  pekangas
 * Added support for song loop counter and non-looping song playback
 *
 * Revision 1.15  1997/04/08 15:48:06  pekangas
 * Added gmpFadeChannel / MIDASfadeMusicChannel functions
 *
 * Revision 1.14  1997/03/29 13:54:26  pekangas
 * Fixed a thread synchronization problem in gmpPlaySong
 *
