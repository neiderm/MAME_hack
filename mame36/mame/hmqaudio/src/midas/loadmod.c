    ...
    gmpModule **_module)
{
    ...
    module->numSamples = 31;                /* set number of samples */
    ...
    {
        if ( ((i & 3) == 0) || ((i & 3) == 3) )
            module->panning[i] = 0x40 - mDefaultStereoSeparation;
        else
            module->panning[i] = 0x40 + mDefaultStereoSeparation;
    }
    ...
    if ( (error = memAlloc(sizeof(gmpSample*) * module->numSamples,
    ...
    for ( i = 0; i < module->numInsts; i++ )
    {
        ...
        modi->slength = BE16(modi->slength);
        if ( maxSample < 2L * (ulong) modi->slength )
            maxSample = 2L * (ulong) modi->slength;
        modi->loopStart = BE16(modi->loopStart);
        modi->loopLength = BE16(modi->loopLength);
    }
    ...
    for ( i = 0; i < module->numInsts; i++ )
    {
        ...
        /* Allocate memory for this instrument structure: (and for one
        ...
        if ( instUsed[i] )
        {
            ...
            if ( (error = memAlloc(sizeof(gmpSample),
                ...
                PASSERR()
            module->instruments[i]->samples[0] = module->samples[i];
        }
        else
        {
        ...
        /* Mark the instrument has no envelopes: */
        ...
        if ( instUsed[i] )
        {
            ...
            if ( (sample->loop1End > 2) &&
                 (sample->loop1End != sample->loop1Start) )
            {
 ...
 *
 * Revision 1.14  1997/07/31 10:56:43  pekangas
 * Renamed from MIDAS Sound System to MIDAS Digital Audio System
 *
 * Revision 1.13  1997/07/30 16:35:48  pekangas
 * Extended Octaves -flag was not properly resetted between loads
 *
 * Revision 1.12  1997/07/09 08:57:58  pekangas
 * Added default stereo separation
 *
 * Revision 1.11  1997/06/30 10:59:52  jpaana
 * Major rewrite on some parts for better IT-support
 *
 * Revision 1.10  1997/06/22 18:09:25  jpaana
 * Made samples independent from instruments. A rather fundamental change, but
 * necessary for .it-support in instrument mode.
 *
 * Revision 1.9  1997/06/01 14:06:47  jpaana
