 ...
 *
 * $Id: mglobals.c,v 1.14 1997/07/31 14:30:16 pekangas Exp $
 *
...
#include "lang.h"
RCSID(const char *mglobals_rcsid = "$Id: mglobals.c,v 1.14 1997/07/31 14:30:16 pekangas Exp $";)
...
int             mSyncScreen;            /* 1 if timer can be synchronized to
                                           screen */
int             mDSoundMode;            /* DirectSound mode, see enum
                                           mDSoundModes */
void           *mDSoundObject;          /* the DirectSound object */
void           *mDSoundHwnd;            /* HWND for DirectSound */
int             mDSoundBufferLength;    /* Buffer length for DirectSound,
                                            not used with emulation */
int             m16bitUlawAutoConvert;  /* Automatically convert 16-bit samples
                                           to u-law for SW mixing */
int             mOutputFilterMode;      /* Output filter mode */
int             mOversampling;          /* Oversampling */
int             mDefaultStereoSeparation; /* Default stereo separation */
int             mLoopModule;            /* Whether modules are looped by
                                           default */
int             mDisableVolumeRamping;    /* Disable volume ramping from
                                             mixing routines */
int             mPreferredMixingMode;     /* Preferred mixing mode */
int             mDisableDSound;           /* Disable DirectSound */
 ...
 *
 * Revision 1.12  1997/07/23 17:27:57  pekangas
 ...
 *
 * Revision 1.6  1997/03/05 19:35:02  pekangas
